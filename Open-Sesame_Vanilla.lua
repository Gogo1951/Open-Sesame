...
-- Constants
local BAG_FULL_MESSAGE_THROTTLE = 10
...
-- Utility function to check if any UI window is open
local function IsAnyWindowOpen()
    local framesToCheck = {
        AuctionFrame,
        BankFrame,
        -- CharacterFrame, -- Uncomment for testing.
        ContainerFrame1,
        ContainerFrame2,
        ContainerFrame3,
        ContainerFrame4,
...
-- Utility function to check available bag space
...
-- Function to play a race and gender-specific sound for a full inventory
...
-- Function to process openable items in the bags
local function ProcessBagItems()
    ...
    for bag = 0, 4 do
        ...
        for slot = 1, numSlots do
            ...
            if itemInfo and AllowedOpenItems[itemInfo.itemID] then
                C_Container.UseContainerItem(bag, slot)
                C_Timer.After(0.5, ProcessBagItems)
                isItemProcessing = false
                return
            end
        end
...
-- Event frame for managing bag updates and loot events
local eventFrame = CreateFrame("Frame")
eventFrame:RegisterEvent("BAG_UPDATE")
eventFrame:RegisterEvent("LOOT_OPENED")
eventFrame:RegisterEvent("UI_ERROR_MESSAGE")
eventFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
eventFrame:RegisterEvent("LOOT_READY") -- Added for auto-loot functionality
...
-- Monitor window states to trigger item processing when windows close
eventFrame:SetScript(
    "OnUpdate",
